...
<div class="justify-content-center text-center col p-0 m-0">
    <div class="row imagemBackgroundPerfilWrapper">
        <div id="imageBackgroundPerfil">
            <span id="bemVindo">
                <div v-if="dataVue.editavel" class="d-contents">Bem vindo,</div><br /><span id="bVNome">{{dataVue.usuarioDados.nome}}</span>
            </span>
            <wm-user-banner @aberto-modal="v => dataVue.abremodal(v)" @recebe-imagem="imgData => dataVue.mudaImagemToCrop(imgData)" :imgcropada="dataVue.imagemCropadaBanner" @configuracoes-crop="conf => dataVue.salvaConfiguracoes(conf)" height_banner="17.15vw" :editavel="dataVue.editavel" :id_usuario="dataVue.idGeral" />
        ...
        <div id="cardsDadosProposta" v-if="dataVue.nivelUsuario != 2">
            <div class="cardsDPWrapper">
                <div class="cardDP pt-0 px-0" v-if="dataVue.editavel">
                    ...
                    <div id="carteiraIcon" @click="() => {dataVue.abremodalCarteira()}">
    ...
    <div class="row justify-content-center text-center">
        <div id="imgcontainer">
            <div v-if="dataVue.Usuario" class="d-contents">
                <wm-user-img :img="dataVue.Usuario.imagem" :width="'14.2vw'" :height="'14.2vw'" :editavel="dataVue.editavel" @aberto-modal="v => dataVue.abremodal(v)" @recebe-imagem="imgData => dataVue.mudaImagemToCrop(imgData)" :imgcropada="dataVue.imagemCropadaUsuario" @configuracoes-crop="conf => dataVue.salvaConfiguracoes(conf)" class_icone="iconeUsuarioTamanho" :id_usuario="dataVue.idGeral" :carregando="dataVue.ipload" />
            </div>
        </div>
    </div>
    <div v-if="dataVue.nivelUsuario == 1" class="d-contents">
        <div class='wrapperStarRating'>
            <star-rating v-model='dataVue.Rating' :increment='0.5' :star-size='dataVue.StarSize' :fixed-points='1' text-class='textoEstrelas' :round-start-rating='false' :read-only='true' :padding='5'></star-rating>
        </div>
    </div>
        ...
        <?php if (BuscaSecaoValor(SecoesEnum::NIVEL_USUARIO) == 1) { ?>
            ...
            <?php } else { ?>
                ...
                <div class="cardQuadrado cemXcem" id="perfilBox">
                    ...
                    <div id="descricaoPerfil" style="padding-left: 5%;padding-right: 5%;">
                    </div>
                </div>
                </div>
            </div>
                        ...
                        <div class="cardQuadrado cemXcem max-heighto" id="statusCard">
                            ...
                            <div class="cardQuadradoBody" id="statusBodyC">
                                ...
                                <div id="membroStatus">
                                    ...
                                    <div v-if="dataVue.editavel" class="d-contents">
                                        <a id="popoverPlano" class="linkPopover" tabindex="0" role="button" data-trigger="focus" data-toggle="popover" data-placement="top" data-content="Como Membro Básico, você tem uma taxa de 15% por serviço terminado, além de não possuir nenhum privilégio.">
...
<wm-modal :visivel="dataVue.modalVisivelEditPerfil" :callback="dataVue.callbackEP" id="modalEdit" height="fit-content" :tem_modal_confirmacao="true">
    ...
    <template v-slot:body>
        <div class="bodyModalEPWrapper">
            <div class="bodyModalEP">
                <wm-input entidade="usuarioDadosEdit" id="inputNomeP" campo="nome" titulo="Nome" class_pai_wrapper="inputPerfil" :obrigatorio="true"></wm-input>
                <?php if (BuscaSecaoValor(SecoesEnum::NIVEL_USUARIO) == 1) { ?>
                    <wm-input entidade="usuarioDadosEdit" id="inputProfissaoP" campo="profissao" titulo="Profissão" class_pai_wrapper="inputPerfil"></wm-input>
                    <div class="inputPerfil" id="tagsInput">
                        ...
                        <tags-input v-model="dataVue.usuarioDadosEdit.tags" label-style="success" delete-key="['46', '8']" placeholder="Competência">
                            <div class="tags-input" slot-scope="{tag, removeTag, inputEventHandlers, inputBindings }">
                                <span v-for="tag in dataVue.tags" class="tags-input-tag">
                                    <span>{{ tag }}</span>
                                    <button type="button" class="tags-input-remove" v-on:click="removeTag(tag)">&times;
                                    </button>
                                </span>
                                <input class="tags-input-text" placeholder="Adicionar Tag..." v-on="inputEventHandlers" v-bind="inputBindings">
                            </div>
                        </tags-input>
                    </div>
                <?php }; ?>
                <wm-textarea entidade="usuarioDadosEdit" id="inputDescricaoP" campo="descricao" titulo="Descrição" class_pai_wrapper="inputPerfil" :maxlength="2000" :row="7"></wm-textarea>
            </div>
            <!-- WMVerificaForm() -->
...
<wm-modal :visivel="dataVue.modalVisivelPlanos" :callback="dataVue.callbackPlanos" id="modalPlano" width="90%" height="fit-content">
    <template v-slot:header>
    ...
    <template v-slot:body>
        <div class="bodyModalPlanoWrapper">
            <wm-card-plano titulo="Membro Padrão" icone="planoPadrao.svg" :botao_situacao="dataVue.situacaoBotao[0]" :plano_number="0" @botao-clickado="(nivel) => {dataVue.darUpgradePlano(nivel)}" preco="Gratuito">
            ...
            <wm-card-plano titulo="Membro Plus" icone="planoPlus.svg" preco="25,00" :plano_number="1" @botao-clickado="(nivel) => {dataVue.darUpgradePlano(nivel)}" :botao_situacao="dataVue.situacaoBotao[1]">
                <template v-slot:descricao>
                    <span>
            ...
            <wm-card-plano titulo="Membro Prime" icone="planoPrime.svg" preco="50,00" :plano_number="2" @botao-clickado="(nivel) => {dataVue.darUpgradePlano(nivel)}" :botao_situacao="dataVue.situacaoBotao[2]">
                <template v-slot:descricao>
                    <span>
            ...
            <wm-card-plano titulo="Membro Master" icone="planoMaster.svg" preco="80,00" :plano_number="3" @botao-clickado="(nivel) => {dataVue.darUpgradePlano(nivel)}" :botao_situacao="dataVue.situacaoBotao[3]">
                <template v-slot:descricao>
                    <span>
...
<wm-modal id="modalCarteira" :visivel="dataVue.modalVisivelCarteira" :callback="dataVue.callbackCarteira" height="40%" width="80%">
    <template v-slot:header>
    ...
    <template v-slot:body>
        <div class="bodyCarteiraModal">
            ...
            <div class="wrapperInputsPagamento">
                ...
                <wm-input entidade="usuarioDadosPagamento" id="inputNumeroCartaoP" campo="numeroCartao" class_pai_wrapper="inputPagamento" :obrigatorio="true" placeholder="Número do Cartão"></wm-input>
                <wm-input entidade="usuarioDadosPagamento" id="inputNomeCartaoP" campo="nome" class_pai_wrapper="inputPagamento" :obrigatorio="true" placeholder="Seu Nome"></wm-input>
                <div class="d-flex" style="width: 100%;">
                    <wm-input entidade="usuarioDadosPagamento" id="inputDataCartaoP" campo="expiracao" class_pai_wrapper="inputPagamento mr-2" :obrigatorio="true" placeholder="MM/AA"></wm-input>
                    <wm-input entidade="usuarioDadosPagamento" id="inputCVCCartaoP" campo="CVC" class_pai_wrapper="inputPagamento" :obrigatorio="true" placeholder="CVC"></wm-input>
                </div>
                ...
                <div class="wrapperInputDinheiro">
                    ...
                    <input id="inputDinheiro" type="number" min="0" max="10000" step="any" value="00.00" />
                </div>
...
<wm-modal id="modalContratar" :visivel="dataVue.modalVisivelContratar" :callback="dataVue.callbackContratar" height="30%" width="50%">
    <template v-slot:header>
...
<wm-modal id="modalAtribuirProjeto" :visivel="dataVue.modalVisivelAtribuirP" :callback="dataVue.callbackAtribuirP" height="60%" width="60%">
    <template v-slot:header>
    ...
    <template v-slot:body>
        <div class="modalAtribuirBody">
            ...
            <div v-else class="d-contents">
                <wm-card-atribuir-projeto :dados_usuario="item" v-for="item in dataVue.meusProjetos" @card-selecionado="(info) => {dataVue.mandarPropostaUsuario(info[0], info[1])}"></wm-card-atribuir-projeto>
            </div>
        </div>
    </template>
    <template v-slot:footer>
...
<wm-modal-botoes-generico id="modalConfirmacao" :visivel="dataVue.modalVisivelControllerConfirmacao" @fechar-modal="(confirmacao) => {dataVue.fechaModalConfirmacao(confirmacao)}" :text_botao_salvar="dataVue.botaoSalvarConfirmacao">
...
<!-- Modal de Crop -->
<wm-crop-modal :img="dataVue.imagemToCrop" :visivel="dataVue.modalVisivelController" @imagem-cropada="img => {dataVue.passaImagemCropada(img);}" @fechar-modal="() => {dataVue.fechamodal()}" :configs="dataVue.configuracoesCrop" />